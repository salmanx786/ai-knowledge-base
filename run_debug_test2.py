...
async def run():
    ...
    async with TestSessionFactory() as session:
        async def _get_db_override():
            print("OVERRIDE CALLED! db bind url:", session.bind.url)
            yield session
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
            ...
            print("Sending second POST request with SAME email")
            response2 = await ac.post("/api/v1/auth/register", json=reg_payload)
